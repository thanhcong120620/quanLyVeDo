...
public class CusHistoryService implements ICusHistoryService{
	...
	@Override
	public List<UserDTO> findAllByUserid2(long user_id, int pageNo, int pageSize) {
...
//			UserEntity User = item.getUser();
			...
			//get region
			long region_id = chRepository.getRegion_id(Id);
			System.out.println("region_id: "+ region_id);
			RegionEntity regionEntity = regionRepository.findOneById(region_id);
			System.out.println("province: "+regionEntity.getNameProvince());

			CusHistoryEntity chEntity = new CusHistoryEntity();
			chEntity.setCodeDrawCH(CodeDrawCH);
			chEntity.setDateCH(DateCH);
			chEntity.setId(Id);
//			chEntity.setProvince(Province);	//sửa lại tên hàm
			chEntity.setRegion(regionEntity);
...
//		userEntity = userRepository.findOneById(userDTO.getId());
		...
		//save data
		chEntity.setRegion(regionEntityByCity);
