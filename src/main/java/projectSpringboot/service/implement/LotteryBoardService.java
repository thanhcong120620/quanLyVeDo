...
public class LotteryBoardService implements ILotteryBoardService{
	...
	public LotteryDTO save(LotteryDTO lotteryDTO) {
		...
		if(lotteryDTO.getId() != null) {
			LotteryBoardEntity oldEntity = lotteryBoardRepository.findById(lotteryDTO.getId()).get();
			lotteryBoardEntity = lotteryBoardConverter.toEntity(lotteryDTO, oldEntity);
		} else {
		...
		lotteryBoardEntity.setRegion(regionEntity);
