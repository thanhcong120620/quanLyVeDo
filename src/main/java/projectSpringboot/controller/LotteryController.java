...
//	private IRegionService regionService;
	...
	public String viewAdminIndex(Model model, HttpServletRequest request) {
		System.out.println("Run url: /admin");
		Principal principal1 = request.getUserPrincipal();
		System.out.println("principal.getName: " + principal1.getName());

		UserDTO dtoUserCl = new UserDTO();
		dtoUserCl.setMailUser(principal1.getName());
		request.getSession().setAttribute("dtoUserSession", dtoUserCl);
		return "redirect:/admin/page/1";
	}

	@GetMapping("/admin/page/{pageNo}")
	public String showEmployeePage(@PathVariable(value = "pageNo") int pageNo, Model model,
...
//		regionService.save(dto);
		System.out.println("oke with id: "+dto.getId());
		// Save Lottery Board
		lbService.save(dto);
	 ...
	 */
	@RequestMapping(value = "/deleteMany/{pageNo}", method = RequestMethod.POST)
	public String deleteMany(@ModelAttribute("deleteTotal") String deleteTotal, Model model, 
